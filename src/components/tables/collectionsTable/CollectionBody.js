...
const renderCell = (accessor, data, url) => {
  ...
  if (accessor === 'title') {
    ...
    const linkUrl = url ? url(data) : data.url || `/edlm-portal/learner/course/${data.id}`;
    return (
  ...
  if (accessor === 'date') {
    const dateString = data[accessor] ? new Date(data[accessor]).toLocaleDateString() : "——";
    return <td key={accessor} className='pl-4'>{dateString}</td>;
  }

  if (accessor === 'competencies') {
     ...
     <tbody className='w-full'>
      {pageData?.map((data, index) => {
       return (
        ...
        {columns.map(({ accessor }) => renderCell(accessor, data, url))}
