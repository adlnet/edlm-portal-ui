...
export default function DevelopmentGoal({ competency, initiallyOpen = true}) {
    ...
    <div className='border rounded-lg border-gray-300 mb-4'>
      <div className='flex flex-row justify-between border-b p-4'>
        <div className='flex gap-2 items-center'>
          {priorityIcon(competency.priority)}
          <h1 className='font-bold text-lg'>{competency.name}</h1>
        </div>
        <button 
          className=''
        ...
        >
          {open ? (<ChevronUpIcon className='h-6 w-6 tex-gray-900'/>) : (<ChevronDownIcon className='h-6 w-6 text-gray-900'/>)}
        </button>
      </div>
      {open && (
        <div>
          {competency.goals?.map((goal, i) => (
            <div key={goal.id}>
